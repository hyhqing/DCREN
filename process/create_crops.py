...
def CreatCrops(base_dir, dataset, crop_type, size, stride, image_suffix, gt_suffix):
    ...
    for name in tqdm(full_file, ncols=100, desc="{}_crops".format(crop_type),
        ...
        if dataset == 'zurich':
            ...
            gt_file = os.path.join(base_dir, 'labels', name.split('n_')[1])
        elif dataset == 'mass':
            image_file = os.path.join(base_dir, 'images', name + '_sat.png')
            gt_file = os.path.join(base_dir, 'labels', name + '_mask.png')
...
def main():
    ...
    parser.add_argument('-d', '--base_dir', type=str, default='C:/Users/admin/Desktop/DCREN/data/',
        help='Base directory for mass Dataset.')
    parser.add_argument('--dataset', type=str, default='mass',
                        choices=['zurich', 'mass'], help='dataset name')

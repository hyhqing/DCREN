...
def direction_process_d1(imgpath, savedir):
    ...
    cv2.imwrite(os.path.join(savedir, os.path.basename(imgpath).split('.')[0] + '_0' + '.png'), dir_array0 * 255)
    cv2.imwrite(os.path.join(savedir, os.path.basename(imgpath).split('.')[0] + '_1' + '.png'), dir_array1 * 255)
    cv2.imwrite(os.path.join(savedir, os.path.basename(imgpath).split('.')[0] + '_2' + '.png'), dir_array2 * 255)
...
def main():
    ...
    parser.add_argument('-d', '--base_dir', type=str, default='C:/Users/admin/Desktop/DCREN/data/',
